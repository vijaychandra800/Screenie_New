...
public class WallPaperDetailsActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        fav_report.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                showReportDialog();
//                Toast.makeText(WallPaperDetailsActivity.this, "This option is not available in demo app", Toast.LENGTH_SHORT).show();
            }
        });
    ...
    //                 /*InputStream is; THIS IS LATEST CODE causing error with http image links
    ...
    private void loadViewed(int pos) {
        try {
            ...
            if (methods.isNetworkAvailable()) {
                new MyTask("", pos, methods.getAPIRequest(Constant.METHOD_WALL_SINGLE, 0, "", "", "", "", Constant.arrayList.get(pos).getId(), "", "", "", "", "", Constant.itemUser.getId(), "")).execute();
            }
        } catch (Exception e) {
            e.printStackTrace();
        }
...
//                        Toast.makeText(WallPaperDetailsActivity.this, "Report is not available in demo app", Toast.LENGTH_SHORT).show();
    ...
    public void loadReportSubmit(String report) {
        if (methods.isNetworkAvailable()) {
            ...
            }, methods.getAPIRequest(Constant.METHOD_REPORT, 0, "", "", "", report, Constant.arrayList.get(position).getId(), "", "", "", "", "", Constant.itemUser.getId(), "wallpaper"));
            loadReport.execute();
        } else {
    ...
    private void getLatestData() {
        if (methods.isNetworkAvailable()) {
            ...
            if (pos_type == 0) {
                requestBody = methods.getAPIRequest(Constant.METHOD_LATEST_WALL, page, color_ids, wallType, "", "", "", "", "", "", "", "", Constant.itemUser.getId(), "");
            } else if (pos_type == 1) {
                requestBody = methods.getAPIRequest(Constant.METHOD_MOST_VIEWED, page, color_ids, wallType, "", "", "", "", "", "", "", "", Constant.itemUser.getId(), "");
            } else if (pos_type == 2) {
                requestBody = methods.getAPIRequest(Constant.METHOD_MOST_RATED, page, color_ids, wallType, "", "", "", "", "", "", "", "", Constant.itemUser.getId(), "");
            } else if (pos_type == 3) {
                requestBody = methods.getAPIRequest(Constant.METHOD_WALLPAPER_BY_CAT, page, color_ids, wallType, cid, "", "", "", "", "", "", "", Constant.itemUser.getId(), "");
            } else if (pos_type == 4) {
                requestBody = methods.getAPIRequest(Constant.METHOD_FAV_WALL, page, color_ids, wallType, "", "", "", "", "", "", "", "", Constant.itemUser.getId(), "wallpaper");
            }
    ...
    private void loadFav(final int posi) {
        if (Constant.isLogged) {
            if (methods.isNetworkAvailable()) {
                ...
                }, methods.getAPIRequest(Constant.METHOD_DO_FAV, 0, Constant.arrayList.get(posi).getId(), "", "", "", "", "", "", "", "", "", Constant.itemUser.getId(), "wallpaper"));
                loadFav.execute();
            } else {
