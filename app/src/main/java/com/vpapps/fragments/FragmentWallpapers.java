...
public class FragmentWallpapers extends Fragment {
    ...
    static FragmentWallpapers newInstance(int position) {
        FragmentWallpapers fragment = new FragmentWallpapers();
    ...
    private void getLatestData() {
        if (pos == 3) {
            ...
            if (methods.isNetworkAvailable()) {
                ...
                if (pos == 0) {
                    requestBody = methods.getAPIRequest(Constant.METHOD_LATEST_WALL, page, color_ids, wallType, "", "", "", "", "", "", "", "", new SharedPref(getActivity()).getUserId(), "");
                } else if (pos == 1) {
                    requestBody = methods.getAPIRequest(Constant.METHOD_MOST_VIEWED, page, color_ids, wallType, "", "", "", "", "", "", "", "", new SharedPref(getActivity()).getUserId(), "");
                } else if (pos == 2) {
                    requestBody = methods.getAPIRequest(Constant.METHOD_MOST_RATED, page, color_ids, wallType, "", "", "", "", "", "", "", "", new SharedPref(getActivity()).getUserId(), "");
                }
                ...
                if (pos == 0) {
                    arrayList = dbHelper.getWallpapers(DBHelper.TABLE_WALL_BY_LATEST, "", wallType, color_ids);
                } else if (pos == 1) {
                    arrayList = dbHelper.getWallpapers(DBHelper.TABLE_WALL_BY_LATEST, "views", wallType, color_ids);
                } else if (pos == 2) {
                    arrayList = dbHelper.getWallpapers(DBHelper.TABLE_WALL_BY_LATEST, "rate", wallType, color_ids);
                }
                setAdapter();
                isOver = true;
                progressBar.setVisibility(View.INVISIBLE);
